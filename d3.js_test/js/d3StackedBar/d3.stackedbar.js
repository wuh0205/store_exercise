function D3StackedBar(options) {
    ...
    var defaultOptions = {
		...
		tooltipOnMouseClick: function(d, element, base) {
			if(typeof base.outTimeId !='undefined' && base.outTimeId){
				clearTimeout(base.outTimeId);
			}
			
			var xPosition = parseInt($(element).attr("x")) + parseInt($(element).attr("width"))/2-base.tooltipWidth/2+base.options.margin.left+5;
			var yPosition = base.y.range()[0]-parseInt($(element).attr("y"))+base.options.margin.bottom+5;
			d3.select(base.options.container+" .tooltip")
				.style("left", xPosition + "px")
				.style("bottom", yPosition + "px")	
			...
			var outTimeId=setTimeout(function () {
				$(base.options.container+" .tooltip").hide();
			}, 3000);

			base.outTimeId=outTimeId;
