...
function D3Core(options) {
    ...
    var defaultOptions = {
		...
		//浮动窗口
		tooltipText: function(d, element) { return "<p>Turnover：<br />x: "+d.x+"<br />y:"+d.y+"<p>"; }
	}	
...
D3Core.prototype = {
	...
	tooltipRender: function() {
		var base = this,
		    charDivId,
		    type=base.options.tipEventType || 'click_timeout';
		switch(type){
			case 'click':
			    charDivId=base.options.container;
                d3.select(charDivId).on('click', function(){
				...
				base.item.on("click", function(d) { base.options.tooltipOnClick(d, this, base); });
				break;
		    case 'click_timeout':
