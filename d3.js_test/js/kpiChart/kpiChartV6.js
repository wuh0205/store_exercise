...
var kpiChart = {
    ...
    height: 410,//400
    svg:null,
    padding: {left: 150, right: 30, top: 60, bottom: 60},
    dataset: [60, 60, 60],
    yDomainMax:6,
    yMax:1,
    xScale:null,
    ...
    initKpi: function () {
        ...
        var setting={
            tipId:'rectTip',
            svg:this.svg,
            rectTranslate:this.rect_translate,
            xMidline:this.xMidline,
            yMidline:this.yMidline,
            companyNameArr:this.companyNameArr,
            tipDetailStore:this.tipDetailStore,
            tipValueStore:this.tipValueStore
        };
        var tipBox=new RectTipBox(setting);
        tipBox.listening();
    },
    initAxis: function () {
        var that=this;
        ...
        //Add the x axis tick content and according to the need for folding line
        var xTextDomArr,zTextDomArr,textDom,d3TextDom;
        xTextDomArr=xBar.selectAll("text")[0];
        for(var i=0 ; i<xTextDomArr.length;i++){
            textDom=xTextDomArr[i];
            d3TextDom=d3.select(textDom);
            d3TextDom.selectAll("tspan")
                .data(that.axisTextToArr(that.xTickNameArr[i]))
                .enter()
                .append("tspan")
                .attr("x", d3TextDom.attr('x'))
        ...
        //Add the z axis tick content and according to the need for folding line
        zTextDomArr=zBar.selectAll("text")[0];
        for(var j=0 ; j<zTextDomArr.length;j++){
            textDom=zTextDomArr[j];
            d3TextDom=d3.select(textDom);
            d3TextDom.selectAll("tspan")
                .data(that.axisTextToArr(that.xTickNameArr[j]))
                .enter()
                .append("tspan")
                .attr("x", d3TextDom.attr('x'))
                .attr("dy", function (d, i) {
        ...
        //According to the folding line sum adjustment text of the label attribute y
        var z_base_y=29,
            add_y=16;
        zBar.selectAll("text")
            .attr('dy', '0em')
            .attr('y', function(d,i){
                tspanSize=$(this).find('tspan').length;
                if(tspanSize==1){
                    return '-'+z_base_y;
                }
                return '-' + (z_base_y+(tspanSize-1)*add_y);
            });
        ...
        //Compute midline value
        var rectSum=this.dataset.length;
        var axisX=$('#'+this.kpiDivName).find('svg>#axis_x');
        var transformStr;
        if(rectSum==5){
            transformStr=axisX.find("g:eq(2)").attr('transform');
            this.xMidline=transformStr.substring(transformStr.indexOf('(') + 1, transformStr.indexOf(','));
        }else if(rectSum==4){
            transformStr=axisX.find("g:eq(3)").attr('transform');
            var xMidline=transformStr.substring(transformStr.indexOf('(') + 1, transformStr.indexOf(','));
            this.xMidline=xMidline/2;

        }else{
            transformStr=axisX.find("g:eq(1)").attr('transform');
            this.xMidline=transformStr.substring(transformStr.indexOf('(') + 1, transformStr.indexOf(','));
        }

        //Compute yOutperLine value
        var axisY=$('#'+this.kpiDivName).find('svg>#axis_y');
        transformStr=axisY.find("g:eq(5)").attr('transform');
        this.yOutperLine=transformStr.substring(transformStr.indexOf(',') + 1, transformStr.indexOf(')'));

        //Compute yMidline value
        axisY=$('#'+this.kpiDivName).find('svg>#axis_y');
        transformStr=axisY.find("g:eq(3)").attr('transform');
        this.yMidline=transformStr.substring(transformStr.indexOf(',') + 1, transformStr.indexOf(')'));
    },
    //???????
    ...
    initBrokenLine:function(){
        var compare=function(str1,str2){
            return parseInt(str1.substring(str1.indexOf('_')+1),str1.length)-parseInt(str2.substring(str2.indexOf('_')+1),str2.length);
        };
        this.checkedList.sort(compare);
        var checkedArr=this.checkedList.show();
        var compyKey,compyNum;
        for(var i in checkedArr){
            compyKey=checkedArr[i];
            compyNum=compyKey.substring(compyKey.indexOf('_')+1,compyKey.length);
            this.drawBrokenLine(this.tabData.get(compyKey),i,compyNum);
        }
    },
    drawBrokenLine:function(_yArr,_num,_compyNum){
        var that=this;
        var yArr = this.yScaleConvert(_yArr);
        var num=_num;
        ...
        var numStr;
        if(num>fillArr.length-1){
            numStr=num.toString();
            num=parseInt(numStr.substring(numStr.length-1));
        }
        ...
        brokenLineG.selectAll("circle")
            ...
            .append("circle")
            .attr("transform", this.rect_translate)
            .attr('data-compyIndex',function(d,i){
                return _compyNum;
            }).
            attr('data-rectIndex',function(d,i){
                return i;
            })
        ...
        this.appendSummary(fillArr[num],_compyNum);
    },
    init:function(obj){
        ...
        this.tipDetailStore=obj.tipDetailStore;
        this.tipValueStore=obj.tipValueStore;
        ...
        for(var i=0;i<yArrStore.length;i++){
            this.tabData.put('companyCheck_'+i,yArrStore[i]);
        }
        this.initKpi();
    },
    ...
    destroyKpi:function(){
        ...
        $("#center_"+this.summaryDivName).remove();
    },
    createSummaryDiv:function(){
        var html="<center id=\"center_"+this.summaryDivName+"\"> <div id=\"kpiChartLegendDiv\" style=\"height: 40px;\"> <div id=\"MyLegend\" width=\"100%\" height=\"100%\"> <div style=\"text-align: center; width: 100%;\"> <div id=\""+this.summaryDivName+"\" width=\"99%\" height=\"99%\" style=\"margin: 0px auto;padding-top: 26px;\"> </div> </div> </div> </div> </center>";
    ...
    yScaleConvert:function(yArr){
        var yMax=this.yMax;
        var y;
        var yDomainMax=this.yDomainMax;
        var returnArr=[];
        //var offset=this.rect_relative_h/100;
        for(var i=0;i<yArr.length;i++){
            y=yArr[i];
            //y=(y-offset)/yMax*yDomainMax;
            returnArr.push(y/yMax*yDomainMax);
        }
        return returnArr;
    },
    xTickNameStrHandle:function(arr){
        for(var i=0;i<arr.length;i++){
            arr[i]=arr[i].replace(/<br>/,' ');
        }
        return arr;
    },
    axisTextToArr:function(str){
        var brArr=[];
        brArr=str.split('<br>');
        //Do newline processing when haven't <br>
        if(brArr.length==1){
            var showLineMax=13,
                content=brArr[0],
                newLineContent,
                ...
                spaceArr=[],
                returnArr=[];
            if(content.length>showLineMax){
                spaceArr=content.split(' ');
                for(var i=0;i<spaceArr.length;i++){
                    if(returnArr.length>0){
                        temp=returnArr.pop();
                        newLineContent=temp+' '+spaceArr[i];
                        if(newLineContent.length<showLineMax){
                            returnArr.push(newLineContent);
                        }else{
                            returnArr.push(temp);
                            returnArr.push(spaceArr[i]);
                        }
                        continue;
                    }
                    returnArr.push(spaceArr[i]);
