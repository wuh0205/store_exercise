...
var kpiChart = {
    ...
    height: 410,//400
    svg:null,
    padding: {left: 150, right: 30, top: 60, bottom: 70},
    dataset: [60, 60, 60],
    yDomainMax:6,
    yMax:1,
    xScale:null,
    ...
    initAxis: function () {
        ...
        var rectNo="";
        if(rectSum%2==0){
            transformStr=axisX.find("g:eq("+(rectSum-1)+")").attr('transform');
            this.xMidline=transformStr.substring(transformStr.indexOf('(') + 1, transformStr.indexOf(','))/2;
        }else{
            rectNo=Math.round(rectSum/2);
            transformStr=axisX.find("g:eq("+(rectNo-1)+")").attr('transform');
