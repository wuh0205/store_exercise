...
function IBMRevenueBar(options) {
    ...
    var defaultOptions = {
        ...
        displayTable: false,
        colors:['rgb(168, 196, 240)','rgb(45, 71, 112)','rgb(96, 152, 240)','rgb(65, 103, 163)','rgb(127, 148, 182)'],
        ...
        tipHtmlPackage:function(tipSubObj,currentKey){
            ...
            for(var i in tipSubObj){
                name = i;
                money = tipSubObj[i];
                if(name=='IBMtotal'||parseInt(money)==0){
                    continue;
                }
                name = i;
                money = tipSubObj[i];
        ...
        tooltipOnClick: function (d, element, base) {
            ...
            if(base.options.showGroup){
                ...
                xPosition = parseInt($(element).attr("x")) + parseInt($(element).attr("width"))/2-base.tooltipWidth/2+base.options.margin.left;
            }
...
IBMRevenueBar.prototype.initData=function(){
    var base=this,
        data=this.options.dataStore.filter(function(e){
            return e;
        });
